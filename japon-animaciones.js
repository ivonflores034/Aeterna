document.addEventListener("DOMContentLoaded", function () {
    // ==========================================================================
    // MOTOR DE VIDEOS ASÍNCRONO - JAPÓN (OPTIMIZADO)
    // ==========================================================================
    const videosJapon = [
        "videos/japon1.mp4", 
        "videos/japon2.mp4", 
        "videos/japon3.mp4",
        "videos/japon4.mp4" // Tus 4 videos cinemáticos de Japón
    ];
    ...
    function inicializarMotorVideosJapon() {
        if (!v1 || !v2) return;

        // Primer video al contenedor activo
        v1.src = videosJapon[0];
        ...
        v2.src = videosJapon[1];

        setInterval(intercalarVideosJapon, 10000);
    }

    function intercalarVideosJapon() {
        // 1. Avanzar en el historial de forma cíclica (0, 1, 2, 3...)
        indiceVideoActual = (indiceVideoActual + 1) % videosJapon.length;
        const proximoIndice = (indiceVideoActual + 1) % videosJapon.length;
        ...
        setTimeout(() => {
            activo.src = videosJapon[proximoIndice];
            activo.load();
        }, 1500); 
    }

    // Arrancar el motor multimedia
    inicializarMotorVideosJapon();
